...
const HomeScreen: React.FC<HomeScreenProps> = ({ navigation }) => {
  ...
  const fetchVehicleType = async () => {
    try {
      ...
      if (response) {
        ...
        console.log("vehicle",response.vehicle_type)
  ...
  const emitDriverLocation = useCallback(() => {
    if (!socket || !isOnDuty || !vehicleType || !location) {
      console.warn("Cannot emit driver location: Missing dependencies.");
      return;
    }
    ...
    try {
      if (driverId !== null) {
        ...
        if (newStatus) {
          ...
          setLocation(loc); // ensure location is set before emitting
          emitDriverLocation();
        } else {
          if (locationInterval.current) clearInterval(locationInterval.current);
